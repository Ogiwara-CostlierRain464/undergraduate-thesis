...
どのケースにおいてもoldに勝ってる
oppositeにてreadでlatestがsnapshotより早いのは、snapshotでは森構造を2度読む必要があるから

oppositeにてwriteがsnapshotがlatestより早いのは、latestでは複数の要素にロックをかける必要がある、またabortするから

oppositeと異なり、directにてreadでlatestがsnapshotと差がないのは、abortがなくなってlatestの待機時間が増えたからだと考えられる。

latestがsnapshotより早いのは、snapshotでは森構造を2度読む必要があるから

abort数が少なくなったので、oppositeと比べdirectではwrite latencyが小さくなっている。

さて、latestのread latencyはoppositeでは最大0.007ms、directでは最大0.02msなのに対し、write latencyは0.8ms、0.3ms
トレードオフの関係。
また、readに比べwriteは10倍程度大きなレイテンシとなる。
