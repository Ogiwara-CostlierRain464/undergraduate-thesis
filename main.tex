...
% 「高精度なロボットにはデータベースの技術が必要だ！！！！」ということを伝えたい
...
\begin{enumerate}
  \item thread: 合計スレッド数
  \item joint: フレームの数
  \item iter: 各スレッドで操作を行う回数。操作は読み込み専用スレッドならlookupTransform(ただしlatestの場合はlookupLatestTransform)、書き込み専用スレッドならsetTransform(ただしlatestの場合はsetTransforms)を指す。
  ...
  \item frequency: 各スレッドにて操作を呼び出す周期。操作の呼び出しが完了したのち、1 / frequency秒待機してから再び操作を呼びだす。0に設定するとiter回の操作が終わるまで待機なしで操作を呼び出し続ける。
\end{enumerate}
...
\begin{figure}[h] 
\centering
\includegraphics[width=7cm]{throughput-a}
...
\begin{figure}[h] 
\centering
\includegraphics[width=7cm]{throughput-b}
...
\begin{figure}[h] 
\centering
\includegraphics[width=7cm]{throughput-c}
\caption{YCSB-Cにおけるスレッド数とスループットの関係}
\label{fig:throughput-c}
\end{figure}


YCSB-A/B/C\cite{ycsb}ワークロードを用いて、old、snapshot、latestのそれぞれにおいてスレッド数とスループットの関係について分析する。各パラメータはjoint=10000、iter=10、read\_len=16、write\_len=16、frequency=0で固定し、read\_ratioとthreadを変化させて計測を行った。

% ここはもっと細かく調査しないとな〜〜
YCSB-Aにおいては　latestはoldに比べ最大20倍のスループットとなった。
% なぜlatestはsnapshotに大幅に勝っている？
YCSB-Bにおいては　latestはoldに比べ最大61倍のスループットとなった。

YCSB-Cにおいては　latestはoldに比べ最大140倍のスループットとなった。

latestがsnapshotより高いスループットを出しているのは、snapshot  では森構造を2度読み込む必要があるからだと考えられる。

これにより、細粒度ロックの導入は効果はあったと考えられる。

\section{レイテンシ}
\begin{figure}[h] 
\centering
\includegraphics[width=7cm]{latency-a}
...
\begin{figure}[h] 
\centering
\includegraphics[width=7cm]{latency-b}
...
\begin{figure}[h] 
\centering
\includegraphics[width=7cm]{latency-c}
\caption{YCSB-Cにおけるスレッド数とレイテンシの関係}
\label{fig:latency-c}
\end{figure}


YCSB-A/B/C\cite{ycsb}ワークロードを用いて、old、snapshot、latestのそれぞれにおいてレイテンシとスループットの関係について分析する。各パラメータはjoint=10000、iter=10、read\_len=16、write\_len=16、frequency=0で固定し、read\_ratioとthreadを変化させて計測を行った。

oldはレイテンシとスレッド数が線形比例するのに対し、提案手法ではほとんど影響がない。

\begin{figure}[h] 
\centering
\includegraphics[width=7cm]{latency-frequency}
\caption{制御周期とレイテンシ}
\label{fig:latency-frequency}
\end{figure}


また、一定周期での操作。これは現実のワークロードではよくあるパターンである。
周期を増やすと当然同時に走る操作も増えるため、こうなるのは自明である。


\begin{figure}[h] 
\centering
\includegraphics[width=7cm]{frequency-real}
\caption{制御周期とレイテンシ}
\label{fig:frequency-real}
\end{figure}

逆に、こういう風なシナリオだとoldでは250msになるから現実の
...
snapshotの方がlatestよりdelayが少なくなるのは、「書き込みスレッドが少なく合計の書き込みが数ない状況において　snapshotの方が書き込みlockを一度に取る数が少なく、書き込み専用スレッドが早く書き込みを終えられ、それをすぐに読み込みスレッドが観測できるから」というread\_ratio=0.95における特別な状況だと考えます
