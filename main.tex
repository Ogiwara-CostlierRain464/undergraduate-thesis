...
\begin{algorithm}[H]
	\caption{lookupTransform(target, source, time)}
	\begin{algorithmic}
	\STATE // 最新のデータ
	\IF{time == 0}
	\STATE time = getLatestCommonTime(target, source)
	\ENDIF
	\STATE source\_trans\_accum = $I$
	\STATE frame = source
	\STATE top\_parent = frame
	\WHILE{frame $\neq$ root}
	\STATE (trans, parent) = frame.getTransAndParent(time) 
	\STATE source\_trans\_accum *= trans
	\STATE top\_parent = frame
	\STATE frame = parent
	\ENDWHILE
	\STATE frame = target
		\STATE target\_trans\_accum = $I$
	\WHILE{frame $\neq$ top\_parent}
	\STATE (tarns, parent)
	\ENDWHILE
	\RETURN 0
...
\begin{algorithm}[H]
	\caption{getLatestCommonTime(target, source)}
	\begin{algorithmic}
	\STATE frame = source
