...
%TODO これじゃあまずそう…ネットの記事を持ってきただけ ok
...
%TODO 反証:問題1に適用したのは2PLではなく細粒度ロック ok
%TODO introにも実装した旨を書く
\section{貢献}
...
%TODO https://sice-si.org/conf/si2021/files/SI2021_program_detail_16Dec2021.pdf
...
sourceフレームからtargetフレームへの最新の座標変換を計算できる時刻を取得できたため、取得できた時刻はtime変数に代入され、処理は\textit{lookupTransform}に戻る。
...
%TODO 224倍以上になる理由がわからない！複合的要因？

%224倍以上の差になったのは、同じフレームに対する複数スレッドからの
%キャッシュの取り合いが発生しなくなったから
%既存手法ではshared\_ptrを使っていたが、
%なぜ？joint=10000の時には実際に500倍の差が出ていた
%mutexよりRWLockの方が早いから？実際、こういう計測を行ってみた
%idle waitをする。これに対し、
%mutex vs busy
%idleが発生しないから、という説明でいい？
